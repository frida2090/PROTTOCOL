...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $canManage) {
    ...
    if ($fecha === '' || $actividad === '') {
        ...
        $query = database()->prepare('INSERT INTO calendario (fecha, actividad, descripcion, profesor_id) VALUES (?, ?, ?, ?)');
        $query->execute([$fecha, $actividad, $descripcion ?: null, $user['id']]);
...
$query = database()->prepare('SELECT c.fecha, c.actividad, c.descripcion, u.nombre AS profesor FROM calendario c JOIN usuario u ON u.id = c.profesor_id WHERE c.fecha BETWEEN ? AND ? ORDER BY c.fecha, c.actividad');
